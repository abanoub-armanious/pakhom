...
All three runs landed in the 4-10 publication-quality target range with
zero single-code themes. The Round 6 case (157 codes → 3 substantive
passes → 7 themes with 2-3 subthemes each) is the empirical
demonstration that v2 produces a clean theme/subtheme hierarchy at
realistic codebook scale.
...
breakdowns -- not a coverage failure. If you need *guaranteed* coverage of named
facets, use Mode 3 (Framework Applied), where the facets are the framework
constructs and every construct is reported.
...
The bracket is empirical -- it's derived from the three Phase 60.8 runs
above. A narrow research focus tends to produce fewer codes per coded
entry (more cohesive); a broad focus produces more codes and may
require more clustering passes.

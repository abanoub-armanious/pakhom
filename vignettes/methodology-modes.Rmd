...
#    object) so we don't re-parse the YAML, or pass `config_path =` if
...
**Empirical evidence from Phase 60.8 validation** (Mode 2 on a 250-entry
binge-eating + sleep corpus):

| Run | Research focus | Codes | Substantive passes | **v2 themes** | Single-code | **v1 baseline** |
|-----|---|---:|---:|---:|---:|---:|
| 1 | medication × sleep × binge | 40 | 1 | **6** | 0% | 69 (87% single-code) |
| 2 | medication / review-path | 47 | 1 | **10** | 0% | 117 |
| 6 | emotional triggers | 157 | **3** | **7** | 0% | 154 (92% single-code) |
...
The Round 6 case (157 codes → 3 substantive passes → 7 themes with 2-3
...
For details on per-theme quality assessment, see
`pakhom/notes/strategic_audit/PHASE_60_9_DEEP_DIVE.md`.

### Dynamic clustering depth, the AI analyst, and the honesty layer (Phases 61--63)
...
**The AI is the analyst; the package is its calculator (Phase 61).** Before
...
three Phase 60.8 validation runs (above), so treat the numbers as rough
...
Phase 57 (re-validation on a 9,178-entry binge-eating + sleep + medication
corpus) found a recurring failure mode you should know about before
choosing Mode 2:
...
Concretely: the Phase 57 run configured `research_focus =
"medication × sleep × binge eating interactions"` against r/BingeEatingDisorder
posts and comments. The corpus's actual dominant signal was generic
binge-eating affect (purging, shame, emotional eating). Mode 2's
Phase 52 HAC + AI tree walk faithfully recovered the dominant signal --
417 themes, organized cleanly. But of those 417, only **2** named all
three of {medication, binge, sleep} together, and only **1** had
substantive mass (~56 entries). The load-bearing medication × sleep
interaction finding was buried in the long tail.

(Phase 60 note: the algorithm has since been rewritten as multi-pass
...
describes a v1 run; under v2, the medication × sleep findings should
surface as their own theme rather than vanishing in 417 single-code
buckets. The Phase 60.8 indicative re-validation (n = 3, summarized
earlier in this vignette) found v2 does this; broader validation remains
future work.)
...
question relevance. If 80% of the corpus is about emotional eating,
80% of the themes will likely be about emotional eating, regardless of
what your `research_focus` says.

### What to do about it

1. **Audit your corpus's signal balance first**. If `research_focus`
   is a niche intersection (e.g., "medication × sleep × binge eating"),
...
3. **Refine `research_focus` to match the dominant signal**.
   If your data is mostly about generic binge eating, you can rewrite
   the question as "How does generic binge eating intersect with
   medication and sleep when it does?" The themes will then be about
   binge eating with medication/sleep as discriminating features --
   a publishable framing.
...
dominant signal (e.g., r/BingeEatingDisorder + "binge eating" focus).
...
- **T0.1 quote provenance** — every AI-attributed verbatim claim verified via the four-step ladder; fabrications dropped + logged to `fabrication_log.csv`.
...
- **T0.3 corpus coverage** — Modes 2/3 assert "no silent truncation in the LLM call path"; Mode 1 asserts "no silent skip across themes × provocation categories." Both are surfaced via a coverage card on the HTML report.
...
- **AC1–AC10 (mode-design commitments)** govern *what modes are* and
  *how methodology is enforced*. They are the answer to "why three
  modes? why is there no default?" Documented in
  `pakhom/notes/strategic_audit/SPRINT4_DESIGN.md` Part I and in the
...
- **C1–C8 (rewrite-direction commitments)** govern *how the coding,
  ...
  "why doesn't the AI just stop wherever the prompt says?" Documented
  in `pakhom/notes/strategic_audit/REWRITE_PLAN_PHASE_50_TO_59.md:498-528`
  and surfaced in the README's "rewrite-direction commitments" section.
...
changes (replacing single-pass HAC with multi-pass clustering — Phase
60) are independent.
...
|---|---|
| C1 — AI decides when to stop | `R/saturation_arbiter.R`, `R/theme_algorithm_v2.R` (Phase 60 multi-pass convergence) |

...
(the AI is never handed a fixed menu to pick from) and archived, so a run can be
...
This metadata is the contract that makes pakhom runs **replay-equivalent**:
two runs against the same data + config + framework hash + provider should
produce comparable artifacts. Cross-mode comparisons via `compare_runs()`

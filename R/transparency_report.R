...
#'
#' \itemize{
#'   \item 1.0.0 (Phase 58 / Stage 1A): initial schema. Sections:
...
#' the top; per the OS.6 spec (anti-Jowsey compliance), it maps every
...
#'
#' Phase 58 audit followup C-T8A-1: \code{stamp_methodology_json}
...
#' \code{write_corpus_coverage} with a non-null methodology mode; the
#' Tier 8 H-10 callsite always stamps). Pre-followup readers accessed
...
.tr_read_reflexivity <- function(run_dir) {
  # Pull from run_metadata if present (Phase 58 onward the config is
  # archived there); otherwise fall back to scanning the saved
  # archived config under the run directory.
  meta <- tryCatch(read_run_metadata(run_dir),
                    error = function(e) NULL)
  meta <- .tr_unwrap_payload(meta)
  ...
  list(
    credibility = list(
      ...
      pakhom_mechanisms = c(
        ...
        "Phase 56 AI saturation arbiter (replaces hardcoded thresholds)"
      ),
      run_evidence = list(
    ...
    transferability = list(
      ...
      pakhom_mechanisms = c(
        ...
        "Phase 55 paper-style per-subtheme tables with metric-tagged quotes"
      ),
      run_evidence = list(
...
.tr_methodology_section <- function(meta, meth_label) {
  ...
  paste0(
    ...
    .html_esc(meta$prompt_template_version %||% "pre_phase58"),
    "</code>\n",
    "</div>\n"
  )
}
...
.tr_quote_provenance_section <- function(qp) {
  ...
  paste0(
    ...
    "passes a four-step verification ladder (string match -> normalized ",
    "match -> substring search -> embedding cosine). Fabrications are ",
...
.tr_footer <- function() {
  ...
  paste0(
    "<hr/>\n",
    "<p class='citation'>Generated by pakhom (Sprint-4 OS.6). ",
    ...
    "<p class='citation'><em>Audit followup M-5 deferral:</em> the ",
    "OS.6 spec also envisaged surfacing M1.4 positionality drift, ",
    "AC6 symmetric obligations summary, and the M1.6 counter-narrative ",

...
ai_propose_clustering <- function(leaves, pass_index, prior_history,
  ...
  pass_context <- if (pass_index == 1L) {
    ...
    paste0(
      ...
      "of these clusters should be merged into a larger cluster, OR ",
  ...
  system_prompt <- paste0(
    ...
    "  verdict = 'continue':  At least one useful merge is possible. ",
    ...
    "cluster_assignments to null. Explain WHY no further merge would yield ",
    ...
    "cluster's organizing principle (MERGE it). A lone code that is one ",
    ...
    "- Topical overlap is NOT a unifying principle. Codes about 'sleep' ",
    "and codes about 'binge eating' can co-occur in the same entries ",
  ...
  if (k == 0L) {
    return(lapply(seq_along(final_leaves), function(i) {
      ...
      list(
        ...
        cluster_rationale = "Each leaf was a single code; AI converged at pass 1 with no merges.",

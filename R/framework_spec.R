...
#'   happens to anomaly SEGMENTS in THIS run's output, not edits to the
#'   spec itself. If a researcher wants to add or modify constructs for a
#'   future run, they edit the framework YAML manually based on what the
#'   anomaly clustering reveals + the framework_review.csv produced under
#'   the "revise" policy.
#'
#'   "extend"  -- cluster the anomaly segments inductively into a section
#'                of "emergent themes" parallel to the framework themes
...
#'                the run directory (one row per anomaly segment with
#'                suggested-edit + accepted columns for the researcher to
#'                fill). Right for pilot studies / framework adaptation
#'                where the researcher expects to update the spec for a
#'                future run after inspecting which segments resisted the
...
#'                a dedicated after_framework_coding pause is deferred
#'                to a future phase (it requires resumable runs with
#'                in-flight spec edits, beyond current checkpoint scope).
...
.validate_framework_spec <- function(raw, source_path = NA_character_) {
  ...
  anomaly <- fw$anomaly_handling %||% "extend"

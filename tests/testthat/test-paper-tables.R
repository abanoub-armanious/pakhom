...
test_that(".detect_metric_columns auto-detects numeric, non-internal columns", {
  df <- tibble::tibble(
    ...
    Policy_Rating = c(8L, 6L),
    Like_Count = c(12, 5)
  )
  out <- pakhom:::.detect_metric_columns(df)
  expect_setequal(out, c("Policy_Rating", "Like_Count"))
})
...
test_that(".detect_metric_columns honors explicit override via direct arg", {
  ...
  out <- pakhom:::.detect_metric_columns(df, explicit = c("Policy_Rating", "Score"))
  expect_setequal(out, c("Policy_Rating", "Score"))
})
...
test_that(".detect_metric_columns drops non-existent override columns", {
  df <- tibble::tibble(std_id = "e1", Policy_Rating = 8L)
  out <- pakhom:::.detect_metric_columns(df, explicit = c("Policy_Rating", "Missing"))
  expect_equal(out, "Policy_Rating")
})
...
test_that(".format_metric_tag emits bracketed tag with present metrics only", {
  row <- data.frame(Policy_Rating = 8, Like_Count = NA_real_,
                     Score = 3.5, stringsAsFactors = FALSE)
  out <- pakhom:::.format_metric_tag(row,
                                       c("Policy_Rating", "Like_Count", "Score"))
  expect_equal(out, "[Policy_Rating: 8; Score: 3.5]")
})
...
.make_theme_for_subtheme_test <- function() {
  ...
  list(
    name = "Daily routines",
    description = "How participants weave the scheduling into daily life.",
    subthemes = list(
      create_subtheme(
        name = "Morning routine",
        description = "Taking the scheduling first thing.",
        codes = list(create_code_object(key = "morning", name = "Morning"))
      ),
      create_subtheme(
        name = "Evening routine",
        description = "Taking the scheduling at night.",
...
test_that(".compute_subtheme_statistics returns one record per real subtheme", {
  ...
  data <- tibble::tibble(
    ...
    Policy_Rating = c(8L, 7L, 9L, 6L, 5L, 4L),
  ...
  out <- pakhom:::.compute_subtheme_statistics(
    ...
    metric_cols = c("Policy_Rating", "Like_Count"),
  ...
  expect_equal(morning$metric_stats$Policy_Rating$median, 8)
  expect_equal(morning$metric_stats$Policy_Rating$mean,   8)

  evening <- out[["Evening routine"]]
  expect_equal(evening$n, 3L)
  expect_equal(evening$metric_stats$Policy_Rating$median, 5)
})
...
test_that(".compute_subtheme_statistics handles missing subtheme_assignments", {
  ...
  data <- tibble::tibble(
    ...
    Policy_Rating = c(8L, 6L, 7L)
  )
  out <- pakhom:::.compute_subtheme_statistics(
    theme, data, metric_cols = "Policy_Rating", quotes_per_subtheme = 1L
  )
...
test_that(".compute_subtheme_statistics example quotes carry metric tags", {
  ...
  data <- tibble::tibble(
    ...
    std_text = c(
      ...
      "I review my plan every evening before I log off because that is when I have time to focus on the week ahead."
    ),
    sentiment_score = c(0.1, -0.1),
    Policy_Rating = c(9L, 5L),
  ...
  expect_match(out$`Morning routine`$example_quotes[1], "Policy_Rating: 9")
  expect_match(out$`Evening routine`$example_quotes[1], "Policy_Rating: 5")
})
...
test_that(".build_subtheme_summary_table emits a row per subtheme + metric cols", {
  ts <- list(
    metric_cols = c("Policy_Rating", "Like_Count"),
    subtheme_stats = list(
      `Morning` = list(name = "Morning", description = "AM shift", n = 3L,
                        metric_stats = list(
                          Policy_Rating = list(median = 8, mad = 1,
                                              mean = 7.8, sd = 0.9),
                          Like_Count  = list(median = 10, mad = 2,
                                              mean = 11, sd = 1.5)
                        ),
                        example_quotes = c("first morning quote [Policy_Rating: 8]"))
    )
  )
...
test_that("export_theme_subtheme_summary_csvs writes per-theme + master CSVs", {
  ...
  theme_stats <- list(
    `Theme A` = list(
      metric_cols = c("Policy_Rating"),
      subtheme_stats = list(
        `Sub 1` = list(
          name = "Sub 1", description = "First sub", n = 3L,
          metric_stats = list(
            Policy_Rating = list(median = 8, mad = 1, mean = 7.5, sd = 0.7,
                                 n_observed = 3L)
          ),
          example_quotes = c("Quote one [Policy_Rating: 8]")
        ),
        `Sub 2` = list(
          name = "Sub 2", description = "Second sub", n = 2L,
          metric_stats = list(
            Policy_Rating = list(median = 5, mad = 1, mean = 5, sd = 1,
                                 n_observed = 2L)
          ),
          example_quotes = c("Quote two [Policy_Rating: 5]")
        )
      )
    ),
  ...
  expect_true("Policy_Rating_median" %in% names(per_theme))
  expect_true("Policy_Rating_mad"    %in% names(per_theme))
  expect_true("Policy_Rating_mean"   %in% names(per_theme))
  expect_true("Policy_Rating_sd"     %in% names(per_theme))
...
test_that("aggregate_theme_statistics attaches subtheme_stats + metric_cols", {
  ...
  expect_true("Policy_Rating" %in% out[["Theme T"]]$metric_cols)
  expect_equal(out[["Theme T"]]$subtheme_stats$`Sub S`$metric_stats$Policy_Rating$median, 7)
})
